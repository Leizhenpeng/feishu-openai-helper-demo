...
import {validateLicenseKey} from '@/utils/api/app';
...
export const LicenseForm = (props: LicenseFormProps) => {
  ...
  const handleSave = async () => {
    ...
    const toastId = toast('保存中...')
    const { isValid } = await validateLicenseKey(value)
    toast.dismiss(toastId) // 取消 '保存中...' 的toast提示
    if (isValid) {
  ...
  return (
    <>
      ...
      <div className="mt-4 flex justify-end gap-3 px-4 sm:px-0">
        <Button variant="solid" color="white" onClick={handleClear}>
          清除
        </Button>
        <Button variant="solid" color="secondary" onClick={handleSave}>
          保存
        </Button>
      </div>
      ...
      <div>
        ...
        >
          <span className="text-sm">再次购买</span>
        </a>
      </div>
    </>
  )
